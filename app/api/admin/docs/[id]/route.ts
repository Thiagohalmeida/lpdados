...
export async function PUT(
  ...
  try {
    ...
    return NextResponse.json(
      { error: 'Erro ao atualizar documentação', details: error instanceof Error ? error.message : 'Unknown error' },
      { status: 500 }
    );
  }
}
...
export async function DELETE(
  ...
  try {
    ...
    return NextResponse.json(
      { error: 'Erro ao deletar documentação', details: error instanceof Error ? error.message : 'Unknown error' },
      { status: 500 }
    );
  }
}
